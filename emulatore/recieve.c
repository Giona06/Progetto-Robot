...
#define HEADER_DIR 0x03
#define HEADER_VEL 0x04
#define END_BYTE 0xff

uint8_t calculate_checksum(uint8_t header_dir, uint8_t data_dir, uint8_t header_vel, uint8_t data_vel) {
    uint8_t checksum_c = header_dir ^ data_dir ^ header_vel ^ data_vel;
    return checksum_c;
}
...
int unpack_data(HANDLE hSerial) {
    ...
    while (1) {
        ...
        // Follow Line Section
        if (!ReadFile(hSerial, &header_dir, 1, &bytesRead, NULL) || header_dir != HEADER_DIR) {
            printf("Invalid Direction header\n");
            continue;
        }
        
        if (!ReadFile(hSerial, data_dir, 1, &bytesRead, NULL)) {
            printf("Error reading direction data\n");
            continue;
        }
        ...
        // Calculate checksum
        uint8_t calculated_checksum = calculate_checksum(header_dir, data_dir[0], header_vel, data_vel[0]);
